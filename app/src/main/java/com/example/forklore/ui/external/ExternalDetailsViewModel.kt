...
import com.example.forklore.data.repository.MyRecipesRepository
...
class ExternalDetailsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val myRecipesRepository = MyRecipesRepository(application)
    ...
    fun saveRecipe() {
        viewModelScope.launch {
            _recipe.value?.data?.let {
                myRecipesRepository.saveRecipe(Recipe(it.id, it.name, it.thumbnail))
            }
        }
    }
}